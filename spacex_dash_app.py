...
for site in launch_sites:
    opts.append({'label':site, 'value':site})
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        data['class'] = data['class'].apply(label_success)
        data['count'] = 1
        chart_title = 'Total Successful Launches for ' + entered_site
        return px.pie(data, values='count', names='class', title=chart_title)
...
def get_scatter_chart(entered_site,entered_range):
    range_min, range_max = entered_range
    if entered_site == 'ALL': data = spacex_df
    else: data = spacex_df[spacex_df['Launch Site'] == entered_site]
    data = data[data['Payload Mass (kg)'].apply(lambda x: x >= range_min and x <= range_max)]
